...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['new_student'])) {
        ...
        $check = "SELECT * FROM students WHERE class='$class' AND fname='$fnname' AND lname='$lname' ";
        ...
        if(mysqli_num_rows($chec) > 0){
            ...
            echo $class;
            $sql = "INSERT INTO students (class , fname, lname, classid, level , count, dir, prov, dist, sect, villa, mother, father, tel1, tel2, section ) VALUES ('$class', '$fnname', '$lname', (SELECT id FROM class WHERE class='$class' ) , (SELECT level FROM class WHERE class='$class' ) , '1' , '$img', '$prov', '$dist', '$sect', '$villa', '$mother', '$father', '$tel1', '$tel2', '$section' )";
        ...
        elseif(!($imgg=="")){
            ...
            $sql = "UPDATE students SET fname='$fnname', lname='$lname', class='$class', dir='$img', prov='$prov', dist='$dist', sect='$sect', villa='$villa', mother='$mother', father='$father', mother='$mother', tel1='$tel1' , tel2='$tel2', section='$section' WHERE id='$student_id'";
            if (mysqli_query($conn, $sql)) {
                echo '<div class="moda fad" id="info" style="width:200px;">  <!--tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true"-->
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <center><p style="color:grey;">Student updated successfully</p></center>
                        </div>
                        <div id="counterr"></div>
                        </div>
                    </div>
            ...
            $query1 = "UPDATE marks SET fname = '$fnname', lname = '$lname' , class = (SELECT class FROM students WHERE id='$student_id'), classid = (SELECT id FROM class WHERE class='$class'), dir='$img' WHERE stid = '$student_id' ";
            $res1 = mysqli_query( $conn , $query1 );
    }
}    
?>
...
<head>
    ...
    <style>
        ...
        *::before {
        ...
        #editButton{line-height: 1.3; background:black; margin-right:50px; width:40px;height:40px;border-radius:50%; border:none; }
        .hidden{ display: none; }
...
<body style="line-height: 1.5;">
    ...
    <div id="stinfo" style="display:none;">
    </div>
    <div id="add">
        <form method="POST" enctype="multipart/form-data" id ="form2">
            <div id="box1" style="">
                <center><h2 style="color:grey; padding:15px 0 0 0;;color:white;">ADD STUDENT</h2></center>
            </div>
            <div id="infor">
                <div id="left">
                    <div id="box2" style="display: flex;overflow:hidden;">
                        <img src="passports/default.png" alt="" style="float:left;width:80px;height:80px;border-radius:50%;margin-left:10px;margin-top:0px;" id="one">
                        <h5 id="one" style="float:right;margin-top:20px;padding-left: 20%;;color:white;">STUDENT INFORMATION</h2>
                    </div>
                    <div id="box3" style="padding-top:10px;">
                        <ul>
                            <li><P>Names:<input type="text" name="student_fname" style="margin-left: 5px;width:60px;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="Firstname"><input type="text" name="student_lname" style="margin-left: 2px;width:60px;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="Lastname"></P></li>
                            <li><p>Class: <select type="text" name="class" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: grey;" required><option value="">Select Class</option><?php $query = "SELECT class FROM class"; $res = mysqli_query( $conn , $query); while($row = $res->fetch_assoc()){ echo "<option value=".$row['class'].">".$row['class']."</option>"; }?></select></p></li>
                            <li><p>Level:  <select type="text" name="nlevel" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: grey;" required><option value="">Select Level</option><option value="Ordinary level">Ordinary level</option><option value="Advanced level">Advanced level</option></select></p></li>
                            <li><p>Section :  <input type="text" name="nsec" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" placeholder="Select Section"></p></li>
                            <li><p><input type="file" name="filename" style="margin-left: 0px;width:89%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" placeholder="Image"></p></li>
                        </ul>
                    </div>
                </div>
                <div id="right">
                ...
                <div id="box5" style="overflow-y: scroll;">
                    ...
                    <ul>
                        <li><p>Province:  <select name="prov" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: grey;" required><option value="">Select Province</option><option value="Kigali City">Kigali City</option><option value="South">South</option><option value="North">North</option><option value="East">East</option><option value="West">West</option></select></p></li>
                        <li><p>District:  <input type="text" name="dist" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="District"></p></li>
                        <li><p>Sector:  <input type="text" name="sect" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="Sector"></p></li>
                        <li><p>Village :  <input type="text" name="villa" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="Village"></p></li>
                    </ul>
                    <h4 style="padding-left: 10px;margin-top:30px;color:white;">Parental information</h2>
                    <hr style="margin-left: 10px;width:90%;background:white;">
                    <ul>
                        <li><P>Fathernames:  <input type="text" name="mothername" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="Father"></P></li>
                        <li><p>Mothernames:  <input type="text" name="fathername" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="Mother"></p></li>
                        <li><p>Telephone-1: <input type="tel" name="tel1" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="Telephone 1"></p></li>
                        <li><p>Telephone-2 : <input type="tel" name="tel2" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="Telephone 2"></p></li>
                        </ul>
                </div>
            </div>
        </div> 
    </form>
    <div id="footer" style="background:chartreuse;padding-bottom:100px;height:10px;margin-top: 10px;">
        <button onclick="hide()" style="background:orange;width:40px;height:40px;border-radius:50%;border: 1px solid #03A9F4;margin-left:10%;margin-top:0px;">&larr;</button>
        <button type="submit" class="btn btn-primary" name="new_student" onclick="reload()" style="background:orange;width:40px;height:40px;border-radius:50%;border: 1px solid #03A9F4;margin-left:56%;margin-top:0px;" form="form2">&rarr;</button>
    </div>
    </div>
    <div id="addst" class="hidden">
        <div id="addstu">
        </div>
        <div id="footer" style="background:chartreuse;padding-bottom:900px;height:10px;margin-top: 10px;">
            <button onclick="hide()" style="background:orange;width:40px;height:40px;border-radius:50%;border: 1px solid #03A9F4;margin-left:10%;margin-top:0px;">&larr;</button>
            <button type="submit" onclick="reload()" class="btn btn-primary" name="updstu" style="background:orange;width:40px;height:40px;border-radius:50%;border: 1px solid #03A9F4;margin-left:56%;margin-top:0px;" form="form3">&rarr;</button>
        </div>
    </div>
    <div>
        <div class="counter-container">
          <div class="counter">
            <?php 
                $tot_students = "SELECT SUM(count) as count FROM students";
                $dothis = $conn->query($tot_students);
                $prefinal = mysqli_fetch_assoc($dothis);
                $final = $prefinal['count'];
            ?>
            <center><h2 style="/*color: #000339;*/color: chartreuse;margin-top:60px;font-size:50px;bolder;" class="count" data-target="<?php echo $final; ?>">0</h3></center>
            <center><h2 style="color:Black;">ALL STUDENTS</h2></center>
          </div>
        </div>
        <!-- <center><h2 style="margin-top:100px;width: 80%;;color:white;">ALL STUDENTS</h2></center> -->
        <button id="editButton" onclick="add()" style="float:right;margin-top:-35px;"><p style="font-size: xx-large;font-weight: bolder;margin:0px 0 0 0;">+</p></button><br>
    </div>


    <div id="change">
        <div id="scroll" style="border: 1px solid aqua;">
            <table class="table table-hover text-center" id="table1">
                ...
                <tbody>
                <?php 
                $sql= "SELECT * FROM students ORDER BY fname ASC";
                $result = mysqli_query($conn , $sql);
                while($row = mysqli_fetch_assoc($result)) {?>
                <tr>
                    <td scope = "td-sm" style="padding-left: 5%;"><img src="passports/<?php echo $row['dir']?>" alt="" style="width:50px;height:50px;border-radius:50%;"></td>
                    <td class = "td-sm" style="padding-left: 15%;"><?php echo $row['fname']. ' ' .$row['lname'] ?></td>
                    <td class = "td-sm" style="padding-left: 11%;">
                        <button style="width: 40px;"class="btn btn-primary" onclick="editst('<?php echo $row['id']; ?>')" style="width:50px;"><p style="margin:0;color:black">EDIT</p><!--<i class="fa fa-pencil">--></i></button>
                        <button class="btn btn-danger btn-sm" onclick="deleteStudent('<?php echo $row['id']; ?>')" style="padding-left:-2px;width:40px;"><p style="font-size:1.5rem; font-weight:700; line-height: 1; text-shadow: 0 1px 0 #fff; background: transparent; cursor:pointer; opacity: 0.5; transition: .3s all ease; text-transform: none; border-radius: 0; background-color: transparent; border: 0; appearance: none; padding: 1rem 0rem; margin: -1rem 0rem -1rem auto;color:black;">&times;</p><!--<i class="fa fa-pencil">--></i></button>
...
Observer.observe(section_counter);
        ...
        // function reload(){
        //     setTimeout(() => {
        //         document.location.href="home.php?students";
        //     }, 4000);
        //     // document.loca.go(0);
        // }

        function hide(){
            addst = document.getElementById('addst');
        ...
        function deleteStudent(studentId) {
            if (confirm("Are you sure you want to delete this student?")) {
                fetch('', {
                    ...
                    .then(data => {
                        // noti('Student deleted successfully');
                    });
                    setTimeout(() => {
                        document.location.href="home.php?students";
                        // history.go(0);
        ...
        function editst(id){
            ...
            .then(data => {
                ...
                <form method="POST" enctype="multipart/form-data" id ="form3">
                        <div id="box1" style="">
                            <center><h2 style="color:grey; padding:15px 0 0 0;color:white;">UPDATE STUDENT INFORMATION</h2></center>
                        </div>
                        <div id="infor">
                            <div id="left">
                                <div id="box2" style="display: flex;overflow:hidden;">
                                ...
                                <div id="box3" style="padding-top:10px;">
                                    <ul>
                                        ...
                                        <li><p>Class: <select "type="text" name="class" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: grey;" required><option selected value="${student.class}">${student.class}</option><?php $query = "SELECT class FROM class"; $res = mysqli_query( $conn , $query); while($row = $res->fetch_assoc()){ echo "<option value=".$row['class'].">".$row['class']."</option>"; }?></select></p></li>
                                        <li><p>Level:  <select type="text" name="nlevel" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: grey;" required><option value="${student.level}">${student.level}</option><option value="Ordinary level">Ordinary level</option><option value="Advanced level">Advanced level</option></select></p></li>
                                        <li><p>Section :  <input type="text" value="${student.section}" name="nsec" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" placeholder="Section"></p></li>
                                        <li><p><input type="file" name="filename" style="margin-left: 0px;width:89%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" placeholder="Image"></p></li>
                                    </ul>
                                </div>
                            </div>
                            <div id="right">
                                <div id="box4" style="">
                                    <h4 style="padding:45px 0px 0 10px;color:white;">OTHER INFORMATION</h1>  
                                </div>
                                <div id="box5" style="overflow-y: scroll;">
                                    <h4 style="padding-left: 10px;">Residence</h2>
                                    <hr style="margin-left: 10px;width:90%;">
                                    <ul>
                                        <li><p>Province:  <select name="prov" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: grey;" required placeholder="Province"><option selected value="${student.prov}">${student.prov}</option><option value="Kigali City">Kigali City</option><option value="South">South</option><option value="North">North</option><option value="East">East</option><option value="West">West</option></select></p></li>
                                    ...
                                    <hr style="margin-left: 10px;width:90%;">
                                    <ul>
                                        <li><P>Fathernames:  <input type="text" value="${student.father}" name="mothername" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="Father"></P></li>
                                        <li><p>Mothernames:  <input type="text" value="${student.mother}" name="fathername" style="margin-left: 5px;width:57%;background: transparent;border: 0px;border-bottom: 1px solid red;color: white;" required placeholder="Mother"></p></li>
                                        ...
                                        <input type="hidden" name="olddir" value="${student.dir}"/>
